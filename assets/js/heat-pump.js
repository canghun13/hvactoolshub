(() => {
  ...
  const calculate = () => {
    ...
    const loadUnit = form.querySelector('[data-load-unit]');
    const thermalUnit = form.querySelector('[data-thermal-unit]');
    const heatingOnly = form.querySelectorAll('[data-heating-only]');
    loadUnit.textContent = unit === 'si' ? 'kW thermal' : 'BTU/h';
    thermalUnit.textContent = unit === 'si' ? 'kWh thermal' : 'BTU thermal';
    heatingOnly.forEach(element => { element.hidden = mode !== 'heating'; });
    ...
    message.textContent = mode === 'heating'
      ? 'Estimated electricity use and cost at the stated heating load and COP. The comparison assumes resistance heat at COP 1.'
      : 'Estimated electricity use and cost at the stated cooling load and COP.';
  };
